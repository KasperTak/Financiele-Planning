...
with tab3:
   ...
   # with kolom1:
    def lineaire_hypotheek(hoofdsom, looptijd_maanden, rentepercentage):
        ...
        data = pd.DataFrame({
                    'Soort last': ['Hypotheek','Rentepercentage','Maandelijkse aflossing', 'Totale rente', 'Totale bruto last'],
                    'Bedrag': [hoofdsom,rentepercentage, aflossing, tot_rente,tot_bruto_mndlasten]})
        data['Bedrag'] = data['Bedrag'].apply(lambda x: f"€{x:,.2f}".replace('.',',').replace(',', '.',1))
        data['Bedrag'][1] = data['Bedrag'][1] + '%'
        data['Bedrag'][1] = data['Bedrag'][1].replace("€","").replace(".",",")
        ...
        if st.toggle('**Grafiek** lineaire hypotheek'):
            st.altair_chart(chart, use_container_width=True)
            
      # specifiek maandnummer en zijn gegevens  
        if st.toggle('**Specifiek maandnummer** lineaire hypotheek'):  
            keuze_maand = st.slider('Voer een maandnummer in waarvan u de gegevens wilt weten voor de lineaire hypotheek.',0,int(tijdsperiode),1)
            ...
            data_mnd = pd.DataFrame({
                        'Soort last': ['Hypotheek','Totale aflossing', 'Totale rente', 'Totale brutolast'],
        ...
        if st.toggle('**Hele tabel** lineaire hypotheek'):
        ...
        return tot_rente, tot_bruto_mndlasten
    
    def annuitaire_hypotheek(hoofdsom, looptijd_maanden, rentepercentage):
        looptijd_maanden = int(tijdsperiode)
        maandelijkse_rente = rentepercentage/100/12
        resterende_schuld = hoofdsom
        annuiteit_factor = (maandelijkse_rente * (1 + maandelijkse_rente)**looptijd_maanden) / ((1 + maandelijkse_rente)**looptijd_maanden - 1)
        maandelijkse_betaling = hoofdsom * annuiteit_factor
        ...
        for maand in range(1,looptijd_maanden+1):
            rente = resterende_schuld * maandelijkse_rente
            aflossing = maandelijkse_betaling - rente
            resterende_schuld -= aflossing
            bruto_maandlast = maandelijkse_betaling
        ...
        df_ann = pd.DataFrame({
                    ...
                    'Rente': rentebedragen,
                    'Aflossing': [round(getal, 2) for getal in aflossingen],
                    'Restschuld': [round(getal,2) for getal in resterende_schulden]})
        
        
        data_ann_grafiek = pd.DataFrame({
                    'Maand': list(range(1,looptijd_maanden+1)),
                    'Bruto maandlast': [round(getal,2) for getal in bruto_maandlasten],
                    'Rente': rentebedragen,
                    'Aflossing': [round(getal, 2) for getal in aflossingen],
        ...
        df_ann['Aflossing'] = df_ann['Aflossing'].apply(lambda x: f"€{x:,.2f}".replace('.', ',').replace(',', '.', 1))
        df_ann['Rente'] = df_ann['Rente'].apply(lambda x: f"€{x:,.2f}".replace('.', ',').replace(',', '.', 1))
        df_ann['Restschuld'] = df_ann['Restschuld'].apply(lambda x: f"€{x:,.2f}".replace('.', ',').replace(',', '.', 1))
        df_ann['Bruto maandlast'] = df_ann['Bruto maandlast'].apply(lambda x: f"€{x:,.2f}".replace('.', ',').replace(',', '.', 1))
        
        # tabel met samenvattende gegevens zoals sommaties.
        
        tot_rente_annuiteit = sum(rentebedragen)
        tot_bruto_mndlasten_annuiteit = sum(bruto_maandlasten)
        
        data_ann = pd.DataFrame({
                    'Soort last': ['Hypotheek','Rentepercentage','Maandelijkse aflossing', 'Totale rente', 'Totale brutolast'],
                    'Bedrag': [hoofdsom, rentepercentage, aflossing, tot_rente_annuiteit,tot_bruto_mndlasten_annuiteit]})
        data_ann['Bedrag'] = data_ann['Bedrag'].apply(lambda x: f"€{x:,.2f}".replace('.',',').replace(',', '.',1))
        data_ann['Bedrag'][1] = data_ann['Bedrag'][1] + '%'
        data_ann['Bedrag'][1] = data_ann['Bedrag'][1].replace("€","").replace(".",",")
        
        st.dataframe(data_ann.set_index(data_ann.columns[0]))
        st.caption('**Bovenstaande tabel geeft de uiteindelijke totale kosten weer van de annuïtaire aflossing.**')

        
        
        # grafiek in de functie
        chart1 = alt.Chart(data_ann_grafiek).mark_area(opacity=0.2, interpolate='linear', color='#ADC9F5').encode(
                x='Maand',y='Bruto maandlast')
        
        # Voeg een tweede lijn toe voor de aflossing
        line1 = alt.Chart(data_ann_grafiek).mark_area(opacity=0.2,interpolate='linear',color='#FFAFB0').encode(
                x='Maand',y='Rente')
        
        chart2 = alt.Chart(data_ann_grafiek).mark_line(color='#025AFF').encode(
                x='Maand',y=alt.Y('Bruto maandlast')).properties(width=600,height=400)
                
        line2 = alt.Chart(data_ann_grafiek).mark_line(color='#FD181F').encode(
                x='Maand',y='Rente')       
        # Legenda maken
        legenda = alt.Chart(pd.DataFrame({'label': ['Aflossing', 'Rente'], 'color': ['#025AFF', '#FD181F']})).mark_text().encode(
                        y=alt.Y('label:N', axis=None),
                        color=alt.Color('color', scale=None),
                        text='label'
                    )
        ...
        # Weergeef de grafiek in Streamlit
        if st.toggle('**Grafiek** annuïteitenhypotheek'):
            st.altair_chart(chart, use_container_width=True)
        

        if st.toggle('**Specifiek maandnummer** annuïteitenhypotheek'):  
            keuze_maand = st.slider('Voer een maandnummer in waarvan u de gegevens wilt weten voor de annuïteitenhypotheek.',0,int(tijdsperiode),1)
            tot_aflossing_mnd = data_ann_grafiek['Aflossing'].iloc[:keuze_maand].sum()
            tot_rente_mnd = data_ann_grafiek['Rente'].iloc[:keuze_maand].sum()
            tot_bruto_mndlasten_mnd = data_ann_grafiek['Bruto maandlast'].iloc[:keuze_maand].sum()
        ...
        if st.toggle("**Hele tabel** annuiteitenhypotheek"):
            st.dataframe(df_ann.set_index(df_ann.columns[0]))
            
            # buffer to use for excel writer
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer,engine='xlsxwriter') as writer:
                df_ann.to_excel(writer, sheet_name='Sheet1',index=False)
        #        writer.save()
            buffer.seek(0)
            download2 = st.download_button(label='Download tabel',data=buffer,
                                               file_name='annuitaire_hypotheek.xlsx', 
                                               mime='applicaiton/vnd.ms-excel')
            if download2:
                st.balloons()
        
        return tot_rente_annuiteit, tot_bruto_mndlasten_annuiteit
